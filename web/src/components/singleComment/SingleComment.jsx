...
const SingleComment = (props) => {
    ...
    return (
        <div className='singleComment'>
            {
                (state.user.userId === props.userId || state.user.isAdmin == true) ? <div className="actionContComment">
                    {showAction ? (
                        <>
                            <ChevronUp onClick={() => { setShowAction(!showAction) }} />
                            <div className="commentMenu">
                                <p onClick={(event) => { props.edit(props._id, event) }}>Edit</p>
                                <p onClick={() => { props.del(props._id) }} >Delete</p>
                            </div>
                        </>
                    )
                        : <ChevronDown onClick={() => { setShowAction(!showAction) }} />}
                </div> : null
            }
